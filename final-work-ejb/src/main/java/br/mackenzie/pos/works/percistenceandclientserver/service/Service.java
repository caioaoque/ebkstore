...
import br.mackenzie.pos.works.percistenceandclientserver.util.dto.DTO;

/**
 * A generic service superclass containing the basic CRUD operations.
 * 
 * @param <E>
 *            the type of the entity handled by this service.
 * @param <T>
 *            the type of the {@link DTO} used to build the conditions of the queries for the given entity type.
 */
public abstract class Service<E extends DomainEntity<?>, T extends DTO<E>> {
    ...
    /**
     * This method should be implemented in order to build the conditions of the query.
     * 
     * @param params
     *            the object which will be used to build the conditions and set the parameters of the query.
     * @return a String object that represents the conditions of the query.
     */
    protected abstract String buildConditions(T params);

    /**
     * This method should be used to build the conditions. It will check if the query being built has already a condition and put an AND
     * clause, otherwise it will put a WHERE clause.
     * 
     * @param condition
     *            the new condition of the query. Note that it must not start with an AND or WHERE keyword.
     * @return a String containing an and or where clause based on the given condition.
     */
    protected final String newCondition(final String condition) {
    ...
    protected final String likeCondition(final String propertyName, final String propertyValue) {
        if (propertyValue != null && !propertyValue.isEmpty()) {
            this.parameters.put(propertyName, "%" + propertyValue + "%");
            return this.newCondition("LOWER(" + this.alias + "." + propertyName + ") like :" + propertyName);
        }
        return EMPTY;
    }

    protected final String inCondition(final String propertyName, final Collection<?> propertyValue) {
        if (propertyValue != null && !propertyValue.isEmpty()) {
            this.parameters.put(propertyName, propertyValue);
            return this.newCondition(this.alias + "." + propertyName + " in :" + propertyName);
        }
        return EMPTY;
    }
